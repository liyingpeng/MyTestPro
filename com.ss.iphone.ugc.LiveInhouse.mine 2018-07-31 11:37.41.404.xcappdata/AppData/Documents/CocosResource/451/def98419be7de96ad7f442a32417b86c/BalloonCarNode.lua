...
function BalloonCarNode:init()
    display.loadSpriteFrames(self:getResourcePath("balloon_car.plist"), self:getResourcePath("balloon_car.png"));
    ...
    local ownerAction = cc.Sequence:create({
        cc.CallFunc:create(function(el)
            ...
            el:align(display.LEFT_BOTTOM, -self.layer.ownerNode.width, 0.6 * display.size.height + 300)
            el:setOpacity(255)
            el:setVisible(true)
        end),

        cc.EaseIn:create(cc.MoveTo:create(0.5,cc.p(0.51*display.size.width-0.5*self.layer.ownerNode.width,0.295*display.size.height+300)),1),
        cc.EaseIn:create(cc.MoveTo:create(0.12,cc.p(0.51*display.size.width-0.5*self.layer.ownerNode.width,0.295*display.size.height+300)),1),
        cc.EaseIn:create(cc.MoveTo:create(0.08,cc.p(0.5*display.size.width-0.5*self.layer.ownerNode.width,0.3*display.size.height+300)),1),     
...
function BalloonCarNode:createSand(car)
    -- body
    local sand = cc.ParticleSystemQuad:create(self:getResourcePath("balloon_car_sand.plist"));
...
function BalloonCarNode:createBalloon(car)
    -- body
    local yellowleft= cc.ParticleSystemQuad:create(self:getResourcePath("balloon_car_yellow_balloon_left.plist"));
    local purpleleft= cc.ParticleSystemQuad:create(self:getResourcePath("balloon_car_purple_balloon_left.plist"));
    local whiteleft= cc.ParticleSystemQuad:create(self:getResourcePath("balloon_car_white_balloon_left.plist"));

    local yellowright= cc.ParticleSystemQuad:create(self:getResourcePath("balloon_car_yellow_balloon_right.plist"));
    local purpleright= cc.ParticleSystemQuad:create(self:getResourcePath("balloon_car_purple_balloon_right.plist"));
    local whiteright= cc.ParticleSystemQuad:create(self:getResourcePath("balloon_car_white_balloon_right.plist"));
...
function BalloonCarNode:onExit()
    ...
    display.removeSpriteFrames(self:getResourcePath("balloon_car.plist"), self:getResourcePath("balloon_car.png"))
...
function BalloonCarNode:create(layer, msg)
    local node = BalloonCarNode.new(layer, msg)
    node:init()
    return node
end
