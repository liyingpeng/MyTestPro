...
function KissNode:init()
    display.loadSpriteFrames(self:getResourcePath("kiss.plist"), self:getResourcePath("kiss.png"));
    ...
    local ownerAction = cc.Sequence:create({
        cc.CallFunc:create(function(el, data)
            ...
            el:align(display.CENTER, display.cx, display.cy + 300)
            el:setOpacity(0)
            el:setVisible(true)
        end),
...
function KissNode:onExit()
    printInfo("kiss  exit!!!!!!!!!!!!");
    display.removeSpriteFrames(self:getResourcePath("kiss.plist"), self:getResourcePath("kiss.png"));
end

function KissNode:create(layer, msg)
    local node = KissNode.new(layer, msg)
    node:init()
    return node
end
