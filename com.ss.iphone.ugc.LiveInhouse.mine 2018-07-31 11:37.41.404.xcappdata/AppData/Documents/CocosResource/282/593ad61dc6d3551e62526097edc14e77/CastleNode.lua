...
function CastleNode:init()
    ...
    local ownerAction = cc.Sequence:create({
            ...
            cc.CallFunc:create(function(el,data)
                ...
                el:align(display.CENTER, 0.55*w, h * 0.6 + 80)
                el:setOpacity(255)
                el:setVisible(true)
            end),
            cc.FadeIn:create(1),
    ...
    local chengbao = cc.Sprite3D:create(self:getResourcePath("castle.c3b"))
        :setVisible(true)
        :setScale(0.0005)
        :setPosition3D(cc.vec3(0.65*w,0.52*h, 600.0 * w / 720))
    ...
    local wheelAnimate=cc.Animate3D:create(wheelAnimation)
    ...
    local horseAction = cc.Sequence:create({
        cc.CallFunc:create(function(el)
            el:setPosition3D(cc.vec3(0.5*w,0.47*h, 980.0 * w / 720))
        end),
        cc.Spawn:create({
            ...
            cc.Sequence:create({
                cc.Spawn:create({
                    ...
                    cc.MoveBy:create(0.6,cc.vec3(0.03*w,0,150)),
                    cc.Sequence:create({
                        cc.DelayTime:create(0.6),
                        cc.MoveBy:create(0.3,cc.vec3(0.67*w,0.08*h,-300)),
                        cc.MoveBy:create(0.9,cc.vec3(-0.6*w,0.05*h,-300)),
                    })
                }),
                cc.Spawn:create({
                    cc.RotateBy:create(1.5,cc.vec3(0,120,0)),
                    cc.MoveBy:create(1.5,cc.vec3(-0.1*w,0.018*h,190)),
                }),
                cc.Spawn:create({
                    cc.RotateBy:create(1.5,cc.vec3(0,-30,0)),
                    cc.MoveBy:create(1.5,cc.vec3(0.16*w,-0.15*h,20)),
                }),
                cc.MoveBy:create(1.5,cc.vec3(0,0.2*h,380)),
            })
        })
    })
