...
function VolcanoNode:create(layer, msg)
    local node = VolcanoNode.new(layer, msg) 
    node:init()
    return node
end

function VolcanoNode:init()
    display.loadSpriteFrames(self:getResourcePath("volcano.plist"), self:getResourcePath("volcano.png"));
    local size = {}
    ...
    local yellowWaveHeight = yellowWave:getContentSize().height * yellowWaveScale/2.5
    ...
    local redWaveHeight = redWave:getContentSize().height * redWaveScale/2.5
    ...
    local volcanoHeight = volcano:getContentSize().height * volcanoScale/2.5
    ...
    local volcanoAction = cc.Sequence:create({
        ...
        cc.EaseIn:create(cc.MoveTo:create(0.4, cc.p(0.5 * size.width, 0.5 * volcanoHeight+50)),2.0),
    ...
    local ownerAction = cc.Sequence:create({
        cc.CallFunc:create(function(el, data)
            ...
            el:align(display.CENTER, display.cx, -75)
            el:setVisible(true)
        end),

        cc.EaseIn:create(cc.MoveTo:create(0.4, cc.p(display.cx, 0.5 * volcanoHeight+50)),2.0),

        cc.DelayTime:create(1.4),

        cc.MoveTo:create(2, cc.p(display.cx, yellowWaveHeight + 0.55 * volcanoHeight+50)),

        cc.DelayTime:create(5.4),

        cc.MoveTo:create(2, cc.p(display.cx, -75)),
    ...
    local goldHeapHeight = goldHeap:getContentSize().height * goldHeapScale/2.2;
...
function VolcanoNode:generateGoldCoins(volcanoHeight, waveHeight)
        ...
        --金币粒子0
        local goldEmitter_0 = cc.ParticleSystemQuad:create(self:getResourcePath("gold_0.plist"));
        goldEmitter_0:setPosition(size.width / 2,0.5 * volcanoHeight)
        goldEmitter_0:setAnchorPoint(0.5,0)

         --金币粒子1
        local goldEmitter_1 = cc.ParticleSystemQuad:create(self:getResourcePath("gold_1.plist"));
        goldEmitter_1:setPosition(size.width / 2,0.5 * waveHeight)
        goldEmitter_1:setAnchorPoint(0.5,0)

         --金币粒子2
        local goldEmitter_2 = cc.ParticleSystemQuad:create(self:getResourcePath("gold_2.plist"));
        goldEmitter_2:setPosition(size.width / 2,0.5 * volcanoHeight)
        goldEmitter_2:setAnchorPoint(0.5,0)

         --金币粒子3
        local goldEmitter_3 = cc.ParticleSystemQuad:create(self:getResourcePath("gold_3.plist"));
        goldEmitter_3:setPosition(size.width / 2,0.5 * volcanoHeight)
        goldEmitter_3:setAnchorPoint(0.5,0)

         --金币粒子4
        local goldEmitter_4 = cc.ParticleSystemQuad:create(self:getResourcePath("gold_4.plist"));
        goldEmitter_4:setPosition(size.width / 2,0.5 * volcanoHeight)
        goldEmitter_4:setAnchorPoint(0.5,0)

         --金币粒子5
        local goldEmitter_5 = cc.ParticleSystemQuad:create(self:getResourcePath("gold_5.plist"));
...
function VolcanoNode:onExit()
    ...
    display.removeSpriteFrames(self:getResourcePath("volcano.plist"), self:getResourcePath("volcano.png"));
end
