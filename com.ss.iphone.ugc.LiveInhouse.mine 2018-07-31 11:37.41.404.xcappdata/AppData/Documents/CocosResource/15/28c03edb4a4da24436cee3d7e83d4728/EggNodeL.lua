...
function EggNode:init()
    display.loadSpriteFrames(self:getResourcePath("egg.plist"), self:getResourcePath("egg.png"))
    ...
    local ownerAction = cc.Sequence:create({
            ...
            cc.CallFunc:create(function(el,data)
                ...
                el:align(display.CENTER, display.cx, display.height * 0.8 + 80)
                el:setOpacity(255)
                el:setVisible(true)
            end),
            cc.DelayTime:create(1),
            cc.MoveTo:create(2.4, cc.p(display.cx, 0)),
            cc.CallFunc:create(function(el,data)
...
function EggNode:onExit()
    -- body
    printInfo("egg  exit!!!!!!!!!!!!")
    display.removeSpriteFrames(self:getResourcePath("egg.plist"), self:getResourcePath("egg.png"))
end

function EggNode:create(layer, msg)
    local node = EggNode.new(layer, msg)
    node:init()
    return node
end
