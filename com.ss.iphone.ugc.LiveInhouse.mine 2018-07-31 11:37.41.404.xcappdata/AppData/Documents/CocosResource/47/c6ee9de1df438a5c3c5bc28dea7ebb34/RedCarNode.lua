...
function RedCarNode:init()
    ...
    local ownerAction= cc.Sequence:create({
        cc.CallFunc:create(function(el)
            ...
            el:align(display.LEFT_BOTTOM, display.width, 0.65 * display.height + 300 - 0.5 * 0.5 * el:getContentSize().height)
            el:setOpacity(255)
            el:setVisible(true)
        end),
    ...
    --雪花粒子
    local snow = cc.ParticleSystemQuad:create(self:getResourcePath("red_car_snow.plist"));
...
function RedCarNode:create(layer, msg)
    local node = RedCarNode.new(layer, msg)
    node:init()
    return node
end
