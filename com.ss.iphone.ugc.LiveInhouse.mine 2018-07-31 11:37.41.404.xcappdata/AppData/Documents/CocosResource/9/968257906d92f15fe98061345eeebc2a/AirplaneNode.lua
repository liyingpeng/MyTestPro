...
function AirplanNode:init()
    -- body
    display.loadSpriteFrames(self:getResourcePath("airplane.plist"), self:getResourcePath("airplane.png"))
    ...
    local ownerAction = cc.Sequence:create({
        cc.CallFunc:create(function(el)
            el:align(display.LEFT_BOTTOM, size.width-6,0.5*size.height+owner:getContentSize().height*0.1)
            el:setScale(0.1)
            el:setVisible(true)
        end),
...
function AirplanNode:onExit()
    ...
    display.removeSpriteFrames(self:getResourcePath("airplane.plist"), self:getResourcePath("airplane.png"))
end

function AirplanNode:create(layer, msg)
    local node = AirplanNode.new(layer, msg) 
    node:init()
    return node
end
